...
var Item = function() {
  /* There are three types of items:
      - Powerups - for now, gain HP
      - Point items - gather these to gain more points
      - 'Mines' (for lack of the actual word) - these decrease the player's health---and maybe points
  */
  this.x = width/2; this.y = height/2;
  this.vx = 0; this.vy = 0;
  this.w = 40;
  ...
  this.drawItem = function() {
    ...
    if (this.timer < 425) { this.alpha = 255; }
    else if (this.timer < 430) { this.alpha = 255; }
    else if (this.timer < 450) { this.alpha = 0; }
    else if (this.timer < 455) { this.alpha = 255; }
    else if (this.timer < 475) { this.alpha = 0; }
    else if (this.timer < 480) { this.alpha = 255; }
    this.c = color(this.r, this.g, this.b, this.alpha);
...
var maxStarW = 8;   // maximum size for background stars
var mode = 0;  // difficulty level: [0,2]
...
function gameOverScreen() {
  background(72,61,139);
  textSize(28); noStroke(); blendMode(LIGHTEST); textStyle(BOLD); fill(255,255,255,180); textAlign(CENTER);
  var s = "GAME OVER\n";
...
// function for adding points, lives
...
function drawHUD(h1, h2) {
  ...
  // Set player colors
  var player1Color = color(0,206,209,180);  // Color for Player 1
  var player2Color = color(0,128,0,180);  // Color   for Player 2
  
  // Draw backgrounds for player info
  fill(player1Color); 
  ellipse(30, 0, 180);
  fill(player2Color); 
  ellipse(width - 30, 0, 180);
  ...
  var h1c = map(h1, 0, 100, 0, 255); 
  var h2c = map(h2, 0, 100, 0, 255);
...
function drawBanana( x, y, w, c, r ) {
  
}

// Fuction to display each players points
function displayPoints( i, x, y, alpha, s ) {
  // When player gets points, display briefly near player (or HUD?).
  var p = i.val;
  textSize(s); textStyle(BOLD); blendMode(NORMAL); stroke(255, 255, 255, alpha); strokeWeight(3);
  fill(0, 255, 150, i.alpha);
  text(p, x, y);
}

// Fuction for displaying combo 
function displayCombo( c, x, y, alpha, s ) {
  // When player gets combo points, display briefly under HUD.
  textSize(s); textStyle(BOLD); blendMode(NORMAL); textAlign(CENTER);
  fill(255, 150, 0, alpha);  stroke(255, 255, 255, alpha); strokeWeight(3);
  var t = "COMBO +" +  str(c);
  text(t, x, y);
}
...
function displayHealth( i, x, y, alpha, s ) {
  ...
  textSize(s); textStyle(BOLD); blendMode(NORMAL);  stroke(255); strokeWeight(3);
  text(str(h), x, y);
}
...
function keyPressed() {
  ...
  if (keyCode == LEFT_ARROW) { 
    p1.left = true; 
    p1.lc = color(180, 0, 0, 255); 
  }
  if (keyCode == UP_ARROW) { 
    p1.up = true; 
    p1.uc = color(180, 0, 0, 255); 
  }
  if (keyCode == RIGHT_ARROW) { 
    p1.right = true; 
    p1.rc = color(180, 0, 0, 255); 
  }
  
  if (key == 'A') { 
    p2.left = true; 
    p2.lc = color(180, 0, 0, 255); 
  }
  if (key == 'W') { 
    p2.up = true; 
    p2.uc = color(180, 0, 0, 255); 
  }
  if (key == 'D') { 
    p2.right = true; 
    p2.rc = color(180, 0, 0, 255); 
  }
}
...
function keyReleased() {
  if (keyCode == LEFT_ARROW) { 
    p1.left = false; 
    p1.lc = color(255, 0, 0, 255); 
  }
  if (keyCode == UP_ARROW) { 
    p1.up = false; 
    p1.uc = color(255, 0, 0, 255); 
  }
  if (keyCode == RIGHT_ARROW) { 
    p1.right = false; 
    p1.rc = color(255, 0, 0, 255); 
  }
  
  if (key == 'A') { 
    p2.left = false; 
    p2.lc = color(255, 0, 0, 255); 
  }
  if (key == 'W') { 
    p2.up = false; 
    p2.uc = color(255, 0, 0, 255); 
  }
  if (key == 'D') { 
    p2.right = false; 
    p2.rc = color(255, 0, 0, 255); 
  }
}